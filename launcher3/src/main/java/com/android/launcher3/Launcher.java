...
public class Launcher extends Activity
    ...
    protected void onInteractionEnd() {
        Log.d(TAG, "onInteractionEnd: mLauncherCallbacks=="+mLauncherCallbacks);
        if (mLauncherCallbacks != null) {
    ...
    public boolean onLongClick(View v) {
        Log.d(TAG, "v.getId()=="+v.getId()+"   v.getParent()=="+v.getParent());
        ...
        if (v instanceof Workspace) {
            Log.d(TAG, "onLongClick: Workspace =="+mWorkspace.isInOverviewMode());
            if (!mWorkspace.isInOverviewMode()) {

                if (!mWorkspace.isTouchActive()) {
                    Log.d(TAG, "onLongClick: mWorkspace.isTouchActive()=="+mWorkspace.isTouchActive());
                    showOverviewMode(true);
        ...
        Log.d(TAG, "onLongClick: v.getTag()=="+v.getTag()+"||||"+(v.getTag() instanceof ItemInfo));
        ...
        Log.d(TAG, "onLongClick: inHotseat=="+inHotseat);
        if (!mDragController.isDragging()) {
            Log.d(TAG, "onLongClick: mDragController.isDragging()=="+mDragController.isDragging());
            Log.d(TAG, "onLongClick: itemUnderLongClick=="+itemUnderLongClick);
            if (itemUnderLongClick == null) {
                // User long pressed on empty space
                mWorkspace.performHapticFeedback(HapticFeedbackConstants.LONG_PRESS,
                        HapticFeedbackConstants.FLAG_IGNORE_VIEW_SETTING);
                Log.d(TAG, "onLongClick: mWorkspace.isInOverviewMode()=="+mWorkspace.isInOverviewMode());
                if (mWorkspace.isInOverviewMode()) {
    ...
    void showOverviewMode(boolean animated) {
        mWorkspace.setVisibility(View.VISIBLE);
        Log.d(TAG, "showOverviewMode: mWorkspace.getState()=="+mWorkspace.getState());
        mStateTransitionAnimation.startAnimationToWorkspace(mState, mWorkspace.getState(),
